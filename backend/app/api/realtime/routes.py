...
@realtime_monitor_bp.route("/cameras", methods=["GET"])
def camera_connect_page():
    return render_template(
        "main/camera_connect.html",
        ipcam_server_url=current_app.config.get("IPCAM_SERVER_URL", "http://localhost:7000"),
        usb_camera_index=current_app.config.get("USB_CAMERA_INDEX", "0")
    )
